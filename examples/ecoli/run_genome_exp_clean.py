...
parser.add_argument("-s", "--scale_embedding",
                    ...
                    type=int,
                    default=2)
parser.add_argument("-l", "--type_laplacian",
                    help="",
                    default='unnormalized')
parser.add_argument("-c", "--min_cc_len",
                    help="",
                    type=int,
                    default=10)
parser.add_argument("-t", "--ovlp_thr_qtle",
                    help="quantile of overlaps to remove",
                    type=float,
                    default=50)
...
dim = args.dim
circular = True
if args.scale_embedding == 0:
    scaled = False
elif args.scale_embedding == 1:
    scaled = 'CTD'
else:
    scaled = True
type_lap = args.type_laplacian
min_cc_len = args.min_cc_len
ovlp_thr_qtle = args.ovlp_thr_qtle
res_dir = args.res_dir

do_plots = False
args_str = "k_{}-d_{}-scaling_{}-ovlpqtile_{}".format(k_nbrs, dim, scaled,
                                                      ovlp_thr_qtle)
print("Experiment : ")
print(args_str)

fig_name = res_dir + "/" + args_str + "-plot-order.pdf"
if os.path.isfile(fig_name):
    sys.exit()
#
k_nbrs = 20  # number of neighbors in the local linear fit in the embedding
dim = 15  # number of dimensions of the embedding
...
ovlp_thr_qtle = 60
#
# prefix = '/home/arecanat/Desktop/test_genome_exp/'
prefix = '/Users/antlaplante/THESE/RobustSeriationEmbedding/test_genome_exp/'
ovlp_fn = prefix + '/results/ovlp.paf'
reads_fn = prefix + '/ecoli_data/reads.fasta'
aln_fn = prefix + '/results/aln.sam'
...
sim_mat = build_sim_mat(ovlp_fn, reads_fn, percentile_thr=0, min_ovlp_len=0)
(iis, jjs, vvs) = find(sim_mat)
ovlp_thr = np.percentile(vvs, ovlp_thr_qtle)
over_thr = np.where(vvs > ovlp_thr)[0]
sim_mat_thr = coo_matrix((vvs[over_thr],
                          (iis[over_thr], jjs[over_thr])),
                         shape=sim_mat.shape,
                         dtype='float64').tocsr()
...
ccs = get_conn_comps(sim_mat_thr, min_cc_len=min_cc_len)
sub_idxs = ccs[0]
new_mat = sim_mat_thr.tolil()[sub_idxs, :]
...
# Use the non-thresholded matrix to assemble together the contigs
this_mat = sim_mat.tolil()[sub_idxs, :]
this_mat = this_mat.T[sub_idxs, :].T


if do_plots:
    (iis, jjs, vvs) = find(this_mat)
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plt.scatter(true_inv_perm[iis], true_inv_perm[jjs])

partial_ords = reord_method.partial_orderings


# this_mat = new_mat
if hasattr(reord_method, 'partial_orderings'):
    total_perm = merge(reord_method.partial_orderings, this_mat,
                       reord_method.embedding, h=100,
                       mode='similarity')
else:
    total_perm = reord_method.ordering
...
if do_plots:
    ...
    # new_mat.shape
            fig = plt.figure()
            for sub_ord in my_ords[:]:
                plt.plot(np.sort(true_inv_perm[sub_ord]), sub_pos[sub_ord],
                         'o')
        else:
                fig = plt.figure()
                plt.plot(np.sort(true_inv_perm[my_ords]), sub_pos[my_ords],
                         'o')

# if len(total_perm[-1]) > 1:
if type(total_perm) == list:
    my_perm = total_perm[-1]
else:
    my_perm = total_perm

if do_plots:
    fig = plt.figure()
    # plt.plot(np.arange(len(total_perm[-1])), true_inv_perm[total_perm[-1]],
    # 'o',
    #          mfc='none')
    plt.plot(np.sort(true_inv_perm[my_perm]), sub_pos[my_perm],
             'o',
             mfc='none')

res_file = res_dir + "/" + args_str + "ordered_ccs.res"
fh = open(res_file, 'a')
print(total_perm, file=fh)
fh.close()

fig_name = res_dir + "/" + args_str + "-plot-order.pdf"
fig = plt.figure()
ax = fig.add_subplot(111)
plt.plot(np.sort(true_inv_perm[my_perm]), sub_pos[my_perm], 'o',
         mfc='none')
plt.savefig(fig_name, bbox_inches='tight')


# score = evaluate_ordering(total_perm, true_perm, circular=True)
# print("circular Kendall-Tau : {}".format(score))
